...
function [repeat_dialog,selslave,selectid,MID,DID,SLA,SLP,SLST] = set_etl_slave_el3xxx(selslave,selectid,MID,DID,SLA,SLP,SLST)
  check_values = 0;
  fd = mopen([getenv('ETLPATH')+'/modules/etherlab_base/src/tcl/'+'etl_scicos_el2xxx.tcl'],'r');
  txt = mgetl(fd,-1);
  mclose(fd);
  
  initvalues = ['array set slavestring { 0 EL3102 1 EL3152 2 EL3162}' 
