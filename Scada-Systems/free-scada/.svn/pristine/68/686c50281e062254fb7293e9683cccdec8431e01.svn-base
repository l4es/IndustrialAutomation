...
public:
	CDockingBoxMessage()
	{
		//Конструктор вызывается раньше, чем инициализируется std::basic_stringstream
		//std::basic_stringstream заменен на CString
		CString smessage;
		smessage.Format(_T("DockingBoxMessage-%u"), GetCurrentProcessId());
		m_message=RegisterWindowMessage(smessage);
		assert(m_message);
		ATLTRACE(_T("%s = %x \n"),smessage, m_message);
