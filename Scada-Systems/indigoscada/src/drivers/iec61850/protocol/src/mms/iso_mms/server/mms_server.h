...
typedef MmsValue* (*ReadVariableHandler) (void* parameter, MmsDomain* domain, char* variableId);
...
void
MmsServer_installReadHandler(MmsServer self, ReadVariableHandler, void* parameter);

void
MmsServer_installWriteHandler(MmsServer self, WriteVariableHandler, void* parameter);

/**
 * A connection handler will be invoked whenever a new client connection is opened or closed
 */
void
MmsServer_installConnectionHandler(MmsServer self, MmsConnectionHandler, void* parameter);
...
void
MmsServer_startListening(MmsServer self);
