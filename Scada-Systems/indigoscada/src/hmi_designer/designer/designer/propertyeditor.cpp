...
void PropertyKeysequenceItem::handleKeyEvent( QKeyEvent *e )
{
    ...
    if (nextKey == QObject::Key_BackTab)
        modifier &= ~QObject::SHIFT;
...
void PropertyKeysequenceItem::setValue()
{
    ...
    if (sequence->hasFocus()
        // if the user wants to use F1 as accelerator, Designer will pop up a QWhatsThat widget
        || (qApp->focusWidget() && qApp->focusWidget()->inherits("QWhatsThat")))
        notifyValueChange();
    setChanged( TRUE );
}
