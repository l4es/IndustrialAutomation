...
wxSize wxFoldPanelItem::m_captionSize(-1,0);
    ...
    , m_userSized(false)
{
    ...
    if (m_captionSize.y==0)
    {
       wxClientDC dc(this);
       wxCoord    w,h;

       dc.GetTextExtent(_T("AWgy"),&w,&h);
       m_captionSize.y=h+6;
    }
    m_captionBar = new wxCaptionBar(this, caption, icons, wxID_ANY, style, wxPoint(0,0),m_captionSize);
...
void wxFoldPanelItem::OnPaint(wxPaintEvent& WXUNUSED(event))
{
    ...
    for(size_t i = 0; i < m_items.GetCount(); i++)
    {
        ...
        wxPen pen(item.GetLineColour(), 1, wxPENSTYLE_SOLID);
#else
