...
#include "wx/keybinder/keybinder.h"
#include "wx/keybinder/menuutils.h"
#include "wx/config.h"
...
bool wxCmd::Save(wxConfigBase *p, const wxString &key, bool bCleanOld) const
{
    ...
    wxString value = wxString::Format(wxT("%s|%s|%s"),
                                    ...
                                    GetDescription().c_str().AsChar(),
                                    shortcuts.c_str().AsChar()
#else
                                    GetName().c_str(),
...
#if wxCHECK_VERSION(2,9,0)
                p->GetName().c_str().AsChar(),
#else
                p->GetName().c_str(),
#endif
                event.GetKeyCode(), event.GetTimestamp());
...
{
    wxString basekey = (key.IsEmpty() ? wxString(wxT("")) : wxString(key));
    bool b = TRUE;
...
#if wxCHECK_VERSION(2,9,0)
            basekey.c_str().AsChar(),
#else
            basekey.c_str(),
#endif
            wxCMD_CONFIG_PREFIX, curr->GetId(), curr->GetType());
...
bool wxKeyProfile::Save(wxConfigBase *cfg, const wxString &key, bool bCleanOld) const
{
    ...
    wxString basekey = key.IsEmpty() ? wxString(wxT("")) : wxString(wxT("/")+key + wxT("/"));
...
bool wxKeyProfile::Load(wxConfigBase *p, const wxString &inkey)
{
    wxString name,desc,key;

    key=wxString(_T("/")+inkey);
    p->SetPath(key);        // enter into this group
    // do we have valid DESC & NAME entries ?
    if (p->HasEntry(key+wxT("/desc")) && p->HasEntry(key+wxT("/name"))) {

        if (!p->Read(key+wxT("/desc"), &desc))
            return FALSE;
        if (!p->Read(key+wxT("/name"), &name))
            return FALSE;
...
wxKeyConfigPanel::~wxKeyConfigPanel()
{
    ...
    for (unsigned int i=0; i < m_pKeyProfiles->GetCount(); i++) {
...
void wxKeyConfigPanel::BuildCtrls()
{
    ...
    if (HasFlag(wxKEYBINDER_NO_DESC_FIELD)) m_pDescLabel=NULL;
    else
    {
#ifdef __WXGTK__
       m_pDescLabel = new wxTextCtrl(this, -1, wxT(""), wxDefaultPosition,wxDefaultSize, wxTE_READONLY | wxTE_MULTILINE);
#else
       m_pDescLabel = new wxStaticText(this, -1, wxT(""), wxDefaultPosition,wxSize(-1, 40), wxSIMPLE_BORDER | wxST_NO_AUTORESIZE);
...
wxSizer *wxKeyConfigPanel::BuildColumn1()
{
    ...
    if (HasFlag(wxKEYBINDER_USE_TREECTRL)) {
        ...
        // add the treectrl
        column1->Add(new wxStaticText(this, -1, _("Commands")+_T(":")), 0, wxGROW | wxALL, 5);
        column1->Add(m_pCommandsTree, 1, wxGROW | wxRIGHT | wxLEFT | wxBOTTOM, 5);

    } else {

        // add a combobox + listbox
        column1->Add(new wxStaticText(this, -1, _("Categories")+_T(":")), 0, wxGROW | wxALL, 5);
        column1->Add(m_pCategories, 1, wxGROW | wxRIGHT | wxLEFT | wxBOTTOM, 5);

        column1->Add(new wxStaticText(this, -1, _("Commands")+_T(":")), 0,
...
wxSizer *wxKeyConfigPanel::BuildColumn2()
{
    ...
    column2->Add(new wxStaticText(this, -1, _("Current shortcuts")+_T(":")), 0, wxGROW | wxALL, 5);
    ...
    column2->Add(new wxStaticText(this, -1, _("New shortcut")+_T(":")), 0, wxGROW | wxALL, 5);
    column2->Add(m_pKeyField, 0, wxGROW | wxLEFT | wxRIGHT, 5);
    column2->Add(new wxStaticText(this, -1, _("Currently assigned to")+_T(":")), 0, wxGROW | wxALL, 5);
...
{
    wxASSERT(m_pKeyProfiles && (n >= 0) && (n < (int)m_pKeyProfiles->GetCount()));
...
#if wxCHECK_VERSION(2,9,0)
                        GetProfile(m_nCurrentProf)->GetName().c_str().AsChar()),
#else
...
void wxKeyConfigPanel::OnAddProfile(wxCommandEvent &)
{
    ...
    while (!valid) {
        ...
        valid = TRUE;
        for (unsigned int j=0; j < m_pKeyProfiles->GetCount(); j++)
