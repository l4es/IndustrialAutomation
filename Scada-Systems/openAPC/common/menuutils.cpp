...
{
//	wxKBLogDebug(wxT("wxMenuWalker::WalkMenuItem - walking on [%s] at level [%d]"),m->GetLabel().c_str(), m_nLevel);
...
{
//	wxKBLogDebug(wxT("wxMenuWalker::WalkMenu - walking on [%s] at level [%d]"),m->GetTitle().c_str(), m_nLevel);
...
void wxMenuWalker::Walk(wxMenuBar *p, void *data)
{
	...
	for (int i=0; i < (int)p->GetMenuCount(); i++) {
		...
		m_nLevel++;
//		wxKBLogDebug(wxT("wxMenuWalker::Walk - walking on [%s] at level [%d]"),p->GetLabelTop(i).c_str(), m_nLevel);
		void *tmp = OnMenuWalk(p, m, data);
...
#if wxCHECK_VERSION(3,0,0)
      wxTreeItemId newId = m_pTreeCtrl->AppendItem(*id, p->GetMenuLabelText(i));
#else
...
#if wxCHECK_VERSION(3,0,0)
      wxTreeItemId newId = m_pTreeCtrl->AppendItem(*id, m->GetItemLabelText(), -1, -1, treedata);
#else
		wxTreeItemId newId = m_pTreeCtrl->AppendItem(*id, m->GetLabel(), -1, -1, treedata);
#endif
		return new wxTreeItemId(newId);
	}
