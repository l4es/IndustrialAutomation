...
struct instData
{
   struct libio_config config;
	int                 m_callbackID;
   unsigned int        useSamples[4],cntSamples[4];
	double              result[4],finalResult[4];
};
...
<oapc-config>\n\
 ...
 <dialogue>\n\
  <general>\n\
   <param>\n\
    <name>samplenum0</name>\n\
    <text>Number of samples (IN0)</text>\n\
    <type>integer</type>\n\
    <default>%d</default>\n\
    <min>2</min>\n\
    <max>1000000000</max>\n\
   ...
   <param>\n\
    <name>samplenum2</name>\n\
    <text>Number of samples (IN2)</text>\n\
    <type>integer</type>\n\
    <default>%d</default>\n\
    <min>2</min>\n\
    <max>1000000000</max>\n\
   ...
   <param>\n\
    <name>samplenum4</name>\n\
    <text>Number of samples (IN4)</text>\n\
    <type>integer</type>\n\
    <default>%d</default>\n\
    <min>2</min>\n\
    <max>1000000000</max>\n\
   ...
   <param>\n\
    <name>samplenum6</name>\n\
    <text>Number of samples (IN6)</text>\n\
    <type>integer</type>\n\
    <default>%d</default>\n\
    <min>2</min>\n\
    <max>1000000000</max>\n\
  ...
  <helppanel>""\n\
   <in0>DATA - Input data to be averaged</in0>\n\
   <in1># - Number of samples to be used for averaging</in1>\n\
   <in2>...</in2>\n\
   <out0>AVG - Averaged value</out0>\n\
   <out2>...</out2>\n\
  </helppanel>\n\
 </dialogue>\n\
</oapc-config>\n";
/** Base64-encoded PNG data of an image that is used as icon for the program flow diagram within the main application*/
static char                 flowImage[]="";
static char                 libname[]="Averaging";
...
OAPC_EXT_API char *oapc_get_config_data(void* instanceData)
{
   ...
   sprintf(xmldescr,xmltempl,flowImage,data->config.m_sampleNum[0],data->config.m_sampleNum[1],
                                       data->config.m_sampleNum[2],data->config.m_sampleNum[3]);
#ifdef _DEBUG
...
OAPC_EXT_API void oapc_set_config_data(void* instanceData,const char *name,const char *value)
{
   ...
   if (strcmp(name,"samplenum0")==0)        data->config.m_sampleNum[0]=atoi(value);
   else if (strcmp(name,"samplenum2")==0)   data->config.m_sampleNum[1]=atoi(value);
   else if (strcmp(name,"samplenum4")==0)   data->config.m_sampleNum[2]=atoi(value);
   else if (strcmp(name,"samplenum6")==0)   data->config.m_sampleNum[3]=atoi(value);
}
...
OAPC_EXT_API char *oapc_get_save_data(void* instanceData,unsigned long *length)
{
   ...
   *length=sizeof(struct libio_config);
   ...
   for (i=0; i<4; i++)
   {
      save_config.m_sampleNum[i]=htonl(data->config.m_sampleNum[i]);
   }
...
OAPC_EXT_API void oapc_set_loaded_data(void* instanceData,unsigned long length,char *loadedData)
{
   ...
   for (i=0; i<4; i++)
   {
      data->config.m_sampleNum[i]=ntohl(save_config.m_sampleNum[i]);
      data->useSamples[i]=data->config.m_sampleNum[i];
   }
}
...
OAPC_EXT_API void* oapc_create_instance2(unsigned long flags)
{
   ...
   for (i=0; i<4; i++)
   {
      data->useSamples[i]=10;
      data->config.m_sampleNum[i]=10;
   }
...
OAPC_EXT_API unsigned long  oapc_set_num_value(void* instanceData,unsigned long input,double value)
{
   ...
   if (input%2==0)
   {
   	data->result[input/2]+=value;
      data->cntSamples[input/2]++;
      if (data->cntSamples[input/2]>=data->useSamples[input/2])
      {
         data->finalResult[input/2]=data->result[input/2]/data->useSamples[input/2];
         data->result[input/2]=0;
         data->cntSamples[input/2]=0;
   	   if (input==0) m_oapc_io_callback(OAPC_NUM_IO0,data->m_callbackID);
         else if (input==2) m_oapc_io_callback(OAPC_NUM_IO2,data->m_callbackID);
         else if (input==4) m_oapc_io_callback(OAPC_NUM_IO4,data->m_callbackID);
         else if (input==6) m_oapc_io_callback(OAPC_NUM_IO6,data->m_callbackID);
      }
   }
   else
   {
      data->useSamples[(input/2)+1]=(unsigned int)value;
      data->cntSamples[(input/2)+1]=0;
      data->result[(input/2)+1]=0;
   }
   return OAPC_OK;
}
...
OAPC_EXT_API unsigned long  oapc_get_num_value(void* instanceData,unsigned long output,double *value)
{
   ...
   if (output % 2==1) return OAPC_ERROR_NO_SUCH_IO;
   data=(struct instData*)instanceData;

   *value=data->finalResult[output/2];
   return OAPC_OK;
}
