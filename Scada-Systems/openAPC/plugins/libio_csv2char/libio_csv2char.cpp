...
struct instData
{
   ...
   int                 m_callbackID;
   char                separator[2];
   char                retChar[MAX_NUM_IOS][MAX_CHAR_SIZE];
};
...
static char                 flowImage[]="";
static char                 libname[]="CSV to Characters";
...
{
   return OAPC_CHAR_IO0|OAPC_CHAR_IO1|OAPC_CHAR_IO2|OAPC_CHAR_IO3|OAPC_CHAR_IO4|OAPC_CHAR_IO5|OAPC_CHAR_IO6|OAPC_CHAR_IO7;
      ...
      else
      {
         ...
         if (*c!=0)
         {
            *c=0;
            ...
            {
               strncpy(data->retChar[i],s,MAX_CHAR_SIZE);
               switch (i)
               {
                  case 0:
                     m_oapc_io_callback(OAPC_CHAR_IO0,data->m_callbackID);
                     break;
                  case 1:
                     m_oapc_io_callback(OAPC_CHAR_IO1,data->m_callbackID);
                     break;
                  case 2:
                     m_oapc_io_callback(OAPC_CHAR_IO2,data->m_callbackID);
                     break;
                  case 3:
                  ...
                  case 5:
                     ...
                     break;
                  case 6:
                     m_oapc_io_callback(OAPC_CHAR_IO6,data->m_callbackID);
                     break;
                  case 7:
                     m_oapc_io_callback(OAPC_CHAR_IO7,data->m_callbackID);
                     break;
                  default:
                     assert(0);
                     break;
...
OAPC_EXT_API unsigned long  oapc_get_char_value(void* instanceData,unsigned long output,unsigned long length,char *value)
{
   ...
   if (length>MAX_CHAR_SIZE) length=MAX_CHAR_SIZE;
   strncpy(value,data->retChar[output],length);
   return OAPC_OK;
}
