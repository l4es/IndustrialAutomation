...
#define mpMIN_X_AXIS_LABEL_SEPARATION 70
#define mpMIN_Y_AXIS_LABEL_SEPARATION 30
...
mpFXY::mpFXY(wxString name, int flags,const bool logDisplay)
      : m_logDisplay(logDisplay)
{
    SetName(name);
    m_flags = flags;
    m_type = mpLAYER_PLOT;
	...
	if (m_visible) 
    {
		...
		if (!m_continuous)
		{
			...
			if (m_pen.GetWidth() <= 1)
			{
				while (GetNextXY(x, y))
				{
                    if (m_logDisplay) y=w.GetYLogCoord(y);
...
void mpScaleX::Plot(wxDC & dc, mpWindow & w)
{
	if (m_visible) {
        ...
        else if (m_labelType == mpX_SCIENTIFIC) 
        {
            if (!m_labelFormat.IsEmpty()) 
            {
                fmt = m_labelFormat;
		...
		for (n = n0; n < end; n += step) {
			...
			if ((p >= startPx) && (p <= endPx)) {
				...
				if ((m_labelType == mpX_NORMAL) || (m_labelType == mpX_SCIENTIFIC))	s.Printf(fmt, n);
...
mpScaleY::mpScaleY(wxString name, int flags, bool ticks, unsigned int type,const bool logDisplay)
         : m_labelType(type)
         , m_YBorderSeparation(60)
         , m_logDisplay(logDisplay)
{
    SetName(name);
    SetFont( (wxFont&) *wxSMALL_FONT);
...
void mpScaleY::Plot(wxDC & dc, mpWindow & w)
{
	if (m_visible) {
		...
		if (m_flags == mpALIGN_LEFT) {
			if (m_drawOutsideMargins)
				orgx = m_YBorderSeparation;
			else
				orgx = w.GetMarginLeft();
		}
		if (m_flags == mpALIGN_RIGHT) {
			if (m_drawOutsideMargins)
				orgx = w.GetScrX() - m_YBorderSeparation;
			else
		...
		if (m_labelFormat.IsEmpty()) 
        {
            if (m_labelType==mpY_NORMAL)
            {
		    	if ((endscale < 1e4) && (endscale > 1e-3)) fmt = wxT("%.2f");
			    else fmt = wxT("%.1e");
            }
            else fmt = wxT("%.2e"); // mpY_SCIENTIFIC
		} 
        else 
        {
			fmt = m_labelFormat;
	...
	//     }
        ...
        if (tx+4>m_YBorderSeparation) m_YBorderSeparation=tx+4;
		for (;n < end; n += step) 
        {
		    ...
		    if ((p >= minYpx) && (p <= maxYpx)) 
            {
			    if (m_ticks) 
                { // Draw axis ticks
				    if (m_flags == mpALIGN_BORDER_LEFT) dc.DrawLine( orgx, p, orgx+4, p);
				    else dc.DrawLine( orgx-4, p, orgx, p); //( orgx, p, orgx+4, p);
                } 
                else 
                {
				    m_pen.SetStyle(wxDOT);
				    dc.SetPen( m_pen);
				    if ((m_flags == mpALIGN_LEFT) && !m_drawOutsideMargins) dc.DrawLine( orgx-4, p, endPx, p);
				    else if ((m_flags == mpALIGN_RIGHT) && !m_drawOutsideMargins) dc.DrawLine( minYpx, p, orgx+4, p);
					else dc.DrawLine( 0/*-w.GetScrX()*/, p, w.GetScrX(), p);
    				m_pen.SetStyle(wxSOLID);
	    			dc.SetPen( m_pen);
		    	}
			    // Print ticks labels

                if (m_logDisplay) s.Printf(fmt,w.GetYLogValue(n));
                else s.Printf(fmt, n);
	    		dc.GetTextExtent(s, &tx, &ty);
		    	labelW = (labelW <= tx) ? tx : labelW;
			    if ((tmp-p+labelHeigth/2) > mpMIN_Y_AXIS_LABEL_SEPARATION) 
                {
				    if ((m_flags == mpALIGN_BORDER_LEFT) || (m_flags == mpALIGN_RIGHT))	dc.DrawText( s, orgx+4, p-ty/2);
				    else dc.DrawText( s, orgx-4-tx, p-ty/2); //( s, orgx+4, p-ty/2);
				    tmp=p-labelHeigth/2;
			    }
		    }
		}
...
mpWindow::mpWindow( wxWindow *parent, wxWindowID id, const wxPoint &pos, const wxSize &size, long flag,double minVal,double maxVal)
    : wxWindow( parent, id, pos, size, flag, wxT("mathplot") )
    , m_yLogMaxVal(maxVal)
    , m_yLogMinVal(minVal)
{
    if (m_yLogMinVal<=0) m_yLogMinVal=0.0000000001;
    m_ylogMinV=log(m_yLogMinVal); 
    double maxV=log(m_yLogMaxVal);
    m_yLogNScale=(maxV-m_ylogMinV)/(m_yLogMaxVal-m_yLogMinVal);
...
#ifndef MATHPLOT_DISABLE_SCREENSHOTS
...
double mpWindow::GetYLogCoord( double n )
{
   if (n<=0) n=0.0000000001;
   return (log(n)-m_ylogMinV)/m_yLogNScale+m_yLogMinVal;
}


double mpWindow::GetYLogValue(double n)
{
   if (n<=0) n=0.0000000001;
   return exp(m_ylogMinV+m_yLogNScale*(n-m_yLogMinVal));
}
...
// Constructor
mpFXYVector::mpFXYVector(wxString name, int flags,const bool logDisplay) : mpFXY(name,flags,logDisplay)
{
    m_index = 0;
    m_minX  = -1;
    m_maxX  = 1;
...
#ifndef MATHPLOT_DISABLE_PRINTING
