...
#define MATHPLOT_DISABLE_PRINTING
#define MATHPLOT_DISABLE_SCREENSHOTS
...
#define mpX_NORMAL     0x00 /** Set label for X axis in normal mode */
#define mpX_TIME       0x01 /** Set label for X axis in time mode: the value is represented as minutes:seconds.milliseconds if time is less than 2 minutes, hours:minutes:seconds otherwise. */
#define mpX_HOURS      0x02 /** Set label for X axis in hours mode: the value is always represented as hours:minutes:seconds. */
#define mpX_DATE       0x03 /** Set label for X axis in date mode: the value is always represented as yyyy-mm-dd. */
#define mpX_SCIENTIFIC 0x04 /** Set label for X axis in scientific mode */

#define mpY_NORMAL     0x00 /** Set label for X axis in normal mode */
#define mpY_SCIENTIFIC 0x04 /** Set label for X axis in scientific mode */
...
public:
    ...
    */
    mpFXY(wxString name = wxEmptyString, int flags = mpALIGN_NE,const bool logDisplay=false);
...
public:
    /** @param name Label to plot by the ruler
        ...
        @param ticks Select ticks or grid. Give TRUE (default) for drawing axis ticks, FALSE for drawing the grid */
    mpScaleY(wxString name = wxT("Y"), int flags = mpALIGN_CENTER, bool ticks = true, unsigned int type = mpX_NORMAL,const bool logDisplay=false);
...
private:
    const unsigned int m_labelType;
                   int m_YBorderSeparation;
    const bool         m_logDisplay;
...
public:
    ...
    mpWindow( wxWindow *parent, wxWindowID id,
                     ...
                     long flags = 0,double minVal=1.0,double maxVal=100.0);
    ~mpWindow();
...
//     wxCoord y2p(double y, bool drawOutside = true); // { return (wxCoord) ( (m_posY-y) * m_scaleY); }
    ...
    */
    ...
    void   InitYLogData();
    double GetYLogCoord(double n);
    double GetYLogValue(double n);
...
private:
    /* values used for logarithmic display */
    const double m_yLogMaxVal;
          double m_yLogMinVal;
          double m_ylogMinV;
          double m_yLogNScale;
...
public:
    /** @param name  Label
        @param flags Label alignment, pass one of #mpALIGN_NE, #mpALIGN_NW, #mpALIGN_SW, #mpALIGN_SE.
    */
    mpFXYVector(wxString name = wxEmptyString, int flags = mpALIGN_NE,const bool logDisplay=false);
