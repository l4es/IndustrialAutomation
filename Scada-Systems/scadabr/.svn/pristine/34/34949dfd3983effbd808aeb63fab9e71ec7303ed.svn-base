 ...
 * the given low limit for a given duration. For example, a user may need to
 * have an event raised when a temperature sinks below some value for 10 minutes
 * or more.
 *
 ...
 * system. In particular, the lowLimitActive field describes whether the point's
 * value is currently below the low limit or not. The eventActive field
 * describes whether the point's value has been below the low limit for longer
...
public class AnalogLowLimitDetectorRT extends TimeDelayedEventDetectorRT<DoubleValueTime> {
    ...
    /**
     * State field. Whether the low limit is currently active or not. This field
     * is used to prevent multiple events being raised during the duration of a
     * single low limit event.
     */
...
*/
    ...
    /**
     * This method is only called when the low limit changes between being
     * active or not, i.e. if the point's value is currently below the low
    ...
    private void changeLowLimitActive() {
        ...
        if (lowLimitActive) // Schedule a job that will call the event active if it runs.
        {
            scheduleJob();
        } else {
    ...
    synchronized public void pointChanged(DoubleValueTime oldValue, DoubleValueTime newValue) {
        ...
        if (newDouble < vo.getLimit()) {
            ...
            if (lowLimitActive) {
                lowLimitInactiveTime = newValue.getTimestamp();
                changeLowLimitActive();
            }
        }
    }
     ...
     * the low limit is active before allowing the event to go active.
     *
     * @param b
     */
    @Override
