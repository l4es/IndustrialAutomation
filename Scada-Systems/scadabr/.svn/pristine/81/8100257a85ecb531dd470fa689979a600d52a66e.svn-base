...
public class DNP3Master {
    ...
    public void doPoll() throws Exception {
        if (reconnecting) {
            System.out.println("[DNP3Master] Trying to reconnect...");
            timeoutCount = 0;
            try {
                try {
                    user.init();
                    reconnecting = false;
                    System.out.println("[DNP3Master] Reconnected!");
                } catch (Exception e) {
                    terminate();
                }
            } catch (Exception e) {
            ...
            if (reconnectNeeded()) {
                ...
                terminate();
                System.out.println("[DNP3Master] Conexão terminada.");
                throw new Exception("[DNP3Master] Poll failed!");
            } else {
                try {
                    System.out.println("[DNP3Master] Poll " + pollingCount
                    ...
                    System.out.println("[DNP3Master] Poll failed! (Error: "
                            + e.getMessage() + ")");
                    timeoutCount++;
                }
