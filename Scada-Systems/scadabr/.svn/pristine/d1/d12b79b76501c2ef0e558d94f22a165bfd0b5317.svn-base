...
abstract public class DifferenceDetectorRT extends TimeDelayedEventDetectorRT {
    ...
    public void initializeState() {
        ...
        if (lastChange + getDurationMS() < System.currentTimeMillis()) // Nothing has happened in the time frame, so set the event active.
        {
            setEventActive(true);
    ...
    synchronized public void setEventActive(boolean b) {
        eventActive = b;
        if (eventActive) // Raise the event.
        {
            raiseAlarm(lastChange + getDurationMS(), createEventContext());
        } else // Deactivate the event.
        {
            clearAlarm(lastChange);
        }
    }
}
