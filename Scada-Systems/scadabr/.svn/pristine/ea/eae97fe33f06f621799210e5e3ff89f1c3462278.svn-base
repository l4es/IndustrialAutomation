 ...
 * below the given CUSUM limit for a given duration. For example, a user may
 * need to have an event raised when a temperature CUSUM sinks below some value
 * for 10 minutes or more.
 *
 * @author Matthew Lohbihler
 */
public class NegativeCusumDetectorRT extends TimeDelayedEventDetectorRT<DoubleValueTime> {
    ...
    /**
     * State field. Whether the negative CUSUM is currently active or not. This
     * field is used to prevent multiple events being raised during the duration
     * of a single negative CUSUM exceed.
     */
...
*/
    ...
    /**
     * This method is only called when the negative CUSUM changes between being
    ...
    private void changeNegativeCusumActive() {
        ...
        if (negativeCusumActive) // Schedule a job that will call the event active if it runs.
        {
            scheduleJob();
        } else {
     ...
     * the negative CUSUM limit is active before allowing the event to go
     * active.
     *
     * @param b
     */
    ...
    synchronized public void setEventActive(boolean b) {
        ...
        if (eventActive) {
            ...
            if (negativeCusumActive) // Ok, things are good. Carry on...
            // Raise the event.
            {
                raiseAlarm(negativeCusumActiveTime + getDurationMS(), createEventContext());
            } else {
                eventActive = false;
            }
        } else // Deactive the event.
        {
            clearAlarm(negativeCusumInactiveTime);
        }
    }
}
