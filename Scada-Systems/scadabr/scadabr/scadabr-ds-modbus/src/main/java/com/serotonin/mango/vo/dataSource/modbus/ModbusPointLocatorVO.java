...
public class ModbusPointLocatorVO extends AbstractPointLocatorVO implements
    ...
    public LocalizableMessage getConfigurationDescription() {
        if (slaveMonitor) {
            return new LocalizableMessageImpl("dsEdit.modbus.dpconn2", slaveId);
        }
        if ((range == RegisterRange.HOLDING_REGISTER || range == RegisterRange.INPUT_REGISTER)
                && modbusDataType == DataType.BINARY) {
            return new LocalizableMessageImpl("dsEdit.modbus.dpconn", slaveId,
                    offset + "/" + bit);
        }
        return new LocalizableMessageImpl("dsEdit.modbus.dpconn", slaveId, offset);
    }
