...
public class SnmpDataSourceRT extends PollingDataSource<SnmpDataSourceVO> {
    ...
    private void doPollImpl(long time) throws IOException {
        ...
        if (request.getVariableBindings().isEmpty()) {
        ...
        if (message != null) {
                ...
                if (dp != null) {
                    ...
                    if (vb.getVariable().isException()) {
                        ...
                        raiseEvent(
                                ...
                                time,
                                true,
                                new LocalizableMessageImpl("event.snmp.oidError",
                    ...
                    raiseEvent(
                            ...
                            time,
                            true,
                            new LocalizableMessageImpl("event.snmp.unknownOid", vb
                                    .getOid(), address));
                }
            }
            ...
            for (DataPointRT requestPoint : requestPoints) {
                error = true;
                raiseEvent(PDU_EXCEPTION_EVENT, time, true,
                        new LocalizableMessageImpl("event.snmp.noBinding",
                                getOid(requestPoint), address));
            }

            if (!error) // Deactivate any existing event.
            {
                returnToNormal(PDU_EXCEPTION_EVENT, time);
            }
        }
    ...
    private LocalizableMessage validatePdu(PDU pdu) {
        if (pdu == null) {
            return new LocalizableMessageImpl("event.snmp.noResponse");
        }

        if (pdu.getErrorIndex() != 0) {
            return new LocalizableMessageImpl("event.snmp.pduOidError", pdu.get(
                    pdu.getErrorIndex() - 1).getOid(), pdu.getErrorStatusText());
        }

        if (pdu.getErrorStatus() != 0) {
            return new LocalizableMessageImpl("event.snmp.pduErrorStatus",
