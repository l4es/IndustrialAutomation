...
public class EBI25InterfaceUpdater {
    ...
    public LocalizableMessage updateLogger(String host, int port, int timeout, int retries, EBI25PointLocatorVO locator) {
        ...
        try {
            ...
            return new LocalizableMessageImpl("dsEdit.ebi25.read.init", e.getMessage());
        } catch (ModbusTransportException e) {
            LOG.warn("Modbus transport", e);
            return new LocalizableMessageImpl("dsEdit.ebi25.read.transport", e.getMessage());
        } catch (ErrorResponseException e) {
            LOG.warn("Modbus error response: " + e.getErrorResponse().getExceptionMessage());
            return new LocalizableMessageImpl("dsEdit.ebi25.read.response", e.getErrorResponse().getExceptionMessage());
        } finally {
